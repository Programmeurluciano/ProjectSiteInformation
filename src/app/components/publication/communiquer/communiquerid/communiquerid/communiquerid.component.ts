...
@Component({
  selector: 'app-communiquerid',
  templateUrl: './communiquerid.component.html',
  styleUrls: ['./communiquerid.component.css']
})
export class CommuniqueridComponent implements OnInit {
  userName:String;
  ...
  constructor(private evenementService: EvenementService,private route:ActivatedRoute,private router:Router
    ,private commentService: CommentairevenementService,private formBuilder: FormBuilder) { }
  ...
  ngOnInit(): void {
    ...
    this.imagePubs;
    this.comments;
    this.Allimagesa(this.route.snapshot.params.id);
  ...
  Allimagesa(id:number)
  {
    this.evenementService.listImagePub(id).subscribe(data=>{
      this.imgPba=data;console.log(data);
    })
  }
